 ...
 * Copied verbatim from the Web project — pure functions, no platform dependency.
 */
...
export function getTaskQuadrant(task: Task, now: Date): Quadrant | null {
  ...
  if (task.importance === 'not_important' && urgent) return 'Q2_NOTIMPORTANT_URGENT'
  if (task.importance === 'important' && !urgent) return 'Q3_IMPORTANT_NOTURGENT'
